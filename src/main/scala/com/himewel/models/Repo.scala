...
object Repo extends Model {
  implicit val decoder: Decoder[Repo] = new Decoder[Repo] {
    def apply(c: HCursor): Decoder.Result[Repo] = for {
  ...
  implicit val encoder: Encoder[Repo] = new Encoder[Repo] {
    def apply(a: Repo): Json = Json.obj(
